...
# to use a different user, just run this command as USER=jdoe ./dish.sh
...
ssh $USER@$HOST "chmod +x $ARTIFACT-instance-1/start && nohup $ARTIFACT-instance-1/start -Dconfig.file=/home/$USER/application.conf -Dhttp.port=9000 > /dev/null &"

ssh $USER@$HOST "kill \`cat $ARTIFACT-instance-2/RUNNING_PID\` && rm -rf $ARTIFACT-instance-2 && cp -R $ARTIFACT $ARTIFACT-instance-2"
ssh $USER@$HOST "chmod +x $ARTIFACT-instance-2/start && nohup $ARTIFACT-instance-2/start -Dconfig.file=/home/$USER/application.conf -Dhttp.port=9001 > /dev/null &"
